...
fn lock_policy_keeps_secrets_environment_owned() {
    ...
    assert!(
        lock.contains("key = \"DATABASE_URL\"\nkind = \"string\"\nrequired = true\nsecret = true")
    );
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "literal secret/connection shape leaked: {forbidden}"
        );
    }
}
...
fn zpkg_does_not_own_runtime_lock_policy() {
    ...
    let Ok(zpkg) = fs::read_to_string(path) else {
        return;
    };
    for forbidden in [
        "wait_timeout_ms",
        ...
        assert!(
            !zpkg.contains(forbidden),
            ".zpkg.toml absorbed runtime lock policy: {forbidden}"
        );
    }
}
